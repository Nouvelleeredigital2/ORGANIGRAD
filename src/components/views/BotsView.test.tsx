...
describe('BotsView', () => {
    ...
    it('liste les bots une fois connecté', async () => {
        ...
        bridgeMock.client = { fetchBots: vi.fn(async () => [BOT]), fetchBotActivation: vi.fn() };
        render(<BotsView />);
    ...
    it("affiche l'état vide avec une invitation à créer le premier bot", async () => {
        bridgeMock.connectionState = 'connected';
        bridgeMock.client = { fetchBots: vi.fn(async () => []), fetchBotActivation: vi.fn() };
        render(<BotsView />);
    ...
    it('ignores an old workspace response after the new workspace has loaded', async()=>{
        ...
        bridgeMock.connectionState='connected';
        bridgeMock.client={fetchBots:()=>new Promise(resolve=>{finish=resolve;}),fetchBotActivation:vi.fn()};
        const view=render(<BotsView/>);
        bridgeMock.client={fetchBots:async()=>[{...BOT,displayName:'Bot workspace B'}],fetchBotActivation:vi.fn()};
