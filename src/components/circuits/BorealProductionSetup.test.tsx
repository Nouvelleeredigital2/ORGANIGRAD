...
import { BorealProductionSetup } from './BorealProductionSetup';
...
const options = {
    projects: [{ name: 'Boréal Production', ref: { projectId: ids[0]!, workspaceId: ids[0]!, sourceApp: 'organigrad', canonicalUrl: 'https://organigrad.example.test/projects/boreal' } }],
...
it('prepares the Boréal Production template with named pilot roles and no schedule', async () => {
    const create = vi.fn().mockResolvedValue(undefined);
    render(<BorealProductionSetup options={options} onCreate={create} />);
    fireEvent.change(screen.getByLabelText('Projet Boréal'), { target: { value: ids[0] } });
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Préparer Boréal Production' }));
    await waitFor(() => expect(create).toHaveBeenCalledWith({
        projectId: ids[0], ericId: ids[2], designId: ids[3], engineId: ids[4], guardianId: ids[5], humanId: ids[1],
    }));
    expect(screen.getByText(/La programmation reste désactivée/i)).toBeInTheDocument();
});
